  ...
  return (
    <Link to={`/tickets/${t.id}`} className="block group">
      ...
      <div className="tr hidden md:grid grid-cols-[120px_1fr_130px_100px_140px_120px] items-center gap-4 px-4 py-3.5 last:border-0">
        <span className="text-xs font-mono text-slate-500 truncate">{t.ticket_number}</span>
        <div className="min-w-0">
          <p className="text-sm font-medium text-slate-200 group-hover:text-blue-400 transition-all duration-200 truncate">{t.title}</p>
          <p className="text-xs text-slate-500 capitalize mt-0.5">{t.category}</p>
        </div>
        <StatusIndicator status={t.status} />
        <AlertBadge priority={t.priority} />
        <span className="text-xs text-slate-500">
          {t.assigned_technician
            ? <span className="flex items-center gap-1 text-slate-400"><User className="w-3 h-3" />{t.assigned_technician.full_name}</span>
            : <span className="text-slate-600">Unassigned</span>}
        </span>
        <span className="text-xs text-slate-500">{formatDistanceToNow(new Date(t.created_at), { addSuffix: true })}</span>
      </div>

      {/* Mobile card */}
      <div className="md:hidden card mb-2 p-4 hover:shadow-md transition-all duration-200" style={{ borderLeft: '3px solid #253046' }}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-slate-400">{page + 1} / {totalPages}</span>
