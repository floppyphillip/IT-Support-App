...
const EMPTY_FORM = {
  ...
  alerts_enabled: false, alert_rule_ids: [],
}
...
function DeviceFormModal({ device, onClose, onSaved, category = 'noc' }) {
  ...
  const [form, setForm] = useState(isEdit ? {
    ...
    alerts_enabled:     device.extra_data?.alerts_enabled  ?? false,
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        extra_data: { snmp_oids: form.snmp_oids, alerts_enabled: form.alerts_enabled, alert_rule_ids: form.alert_rule_ids },
    ...
    <>
      ...
      <div className="w-full max-w-[806px] flex flex-col rounded-2xl shadow-2xl border overflow-hidden"
        ...
        <div className="p-6 space-y-5 flex-1 overflow-y-auto">
          ...
          {/* Alert Rules */}
          <section className="border-t pt-5" style={{ borderColor: 'var(--border)' }}>
            <Toggle label="Alert Rules" checked={form.alerts_enabled} onChange={v => set('alerts_enabled', v)} />
            {form.alerts_enabled && (
              <div className="mt-3">
                  ...
                  {filteredLogs.map(log => {
                    ...
                    return (
                      <div key={log.id} className="flex items-start gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-[13px] font-bold flex-shrink-0 mt-0.5"
                             style={{ background: bg }}>
                          {label}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[17px] font-semibold text-gray-800 truncate">{log.title}</p>
                          <p className="text-[15px] mt-0.5 truncate" style={{ color: 'var(--text-3)' }}>{log.message}</p>
                        </div>
                        <p className="text-[14px] font-mono flex-shrink-0 mt-0.5 text-right whitespace-nowrap" style={{ color: 'var(--text-4)' }}>
...
export default function Devices() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {showAddLink  && <LinkFormModal category="noc" onClose={() => setShowAddLink(false)} onSaved={load} />}
