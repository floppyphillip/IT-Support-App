  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="relative">
        ...
        <input className="input pl-9 w-52" placeholder="Search devices…" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {devices.map((d) => (
            <Link key={d.id} to={`/devices/${d.id}`} className="card p-5 hover:shadow-md transition-all duration-200 group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-900 group-hover:text-blue-600 transition-all duration-200">{d.name}</p>
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs text-slate-500 mb-4">
                {d.last_ping_ms != null && (
                  <span className="flex items-center gap-1"><Activity className="w-3 h-3 flex-shrink-0 text-blue-400" />{d.last_ping_ms}ms</span>
                )}
                {d.cpu_usage != null && (
                  <span className="flex items-center gap-1"><Cpu className="w-3 h-3 flex-shrink-0 text-violet-400" />{d.cpu_usage.toFixed(0)}%</span>
                )}
                {d.memory_usage != null && (
                  <span className="flex items-center gap-1"><HardDrive className="w-3 h-3 flex-shrink-0 text-emerald-400" />{d.memory_usage.toFixed(0)}%</span>
                )}
                {d.location && (
                  <span className="flex items-center gap-1 col-span-2 truncate"><MapPin className="w-3 h-3 flex-shrink-0 text-amber-400" />{d.location}</span>
                )}
              </div>
