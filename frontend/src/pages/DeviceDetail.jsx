...
import { ArrowLeft, Terminal, Activity, RefreshCw, Database, Download } from 'lucide-react'
...
export default function DeviceDetail() {
  ...
  const load = async () => {
    try {
      ...
      setDevice(d)
      setMetrics([...m].reverse()) // chronological order for chart
      setBackups(b)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        <Link to="/devices" className="text-gray-400 hover:text-gray-600"><ArrowLeft className="w-5 h-5" /></Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{device.name}</h1>
          <div className="flex items-center gap-2 mt-1">
            <code className="text-sm text-gray-500">{device.ip_address}</code>
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={ping} className="btn-secondary" disabled={pinging}>
            <Activity className="w-4 h-4" /> {pinging ? 'Pinging…' : 'Ping'}
          </button>
          {device.snmp_enabled && (
          ...
          {device.ssh_enabled && (
            <>
              <button onClick={triggerBackup} className="btn-secondary" disabled={backingUp}>
                <Database className="w-4 h-4" /> {backingUp ? 'Backing up…' : 'Backup Config'}
              </button>
              <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card p-6">
            <h2 className="font-semibold mb-4">Live Metrics</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                { label: 'Latency', value: device.last_ping_ms != null ? `${device.last_ping_ms.toFixed(1)}ms` : '–', color: 'blue' },
                { label: 'CPU', value: device.cpu_usage != null ? `${device.cpu_usage.toFixed(1)}%` : '–', color: 'purple' },
                { label: 'Memory', value: device.memory_usage != null ? `${device.memory_usage.toFixed(1)}%` : '–', color: 'green' },
                { label: 'Disk', value: device.disk_usage != null ? `${device.disk_usage.toFixed(1)}%` : '–', color: 'yellow' },
              ].map(({ label, value, color }) => (
                <div key={label} className={`rounded-lg p-4 bg-${color}-50`}>
            ...
            {metrics.length > 1 && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-gray-500 mb-1">Latency (ms) — last {metrics.length} polls</p>
                ...
                {cpuData.some((d) => d.cpu != null) && (
                  <div>
                    <p className="text-xs text-gray-500 mb-1">CPU % — last {metrics.length} polls</p>
                    <ResponsiveContainer width="100%" height={60}>
          ...
          <div className="card p-6">
            <h2 className="font-semibold mb-4 flex items-center gap-2">
              <Database className="w-4 h-4" /> Config Backups ({backups.length})
            </h2>
            {backups.length === 0 ? (
              <p className="text-sm text-gray-400">No backups yet. Click "Backup Config" to create one.</p>
            ) : (
              <div className="space-y-2">
                {backups.map((backup) => (
                  <div key={backup.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
          ...
          <div className="card p-5 space-y-3 text-sm">
            {[
              ...
              <div key={k} className="flex justify-between gap-2">
                <span className="text-gray-500 flex-shrink-0 capitalize">{k}</span>
                <span className="text-right text-gray-800 capitalize">{v ?? '–'}</span>
              </div>
            ))}
          </div>
        </div>
