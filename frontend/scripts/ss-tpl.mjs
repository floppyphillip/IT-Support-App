...
// Click "From Template" dropdown
await page.evaluate(() => {
  const btns = [...document.querySelectorAll('button')]
  const btn = btns.find(b => b.textContent.includes('From Template'))
  if (btn) btn.click()
})
await new Promise(r => setTimeout(r, 500))
await page.evaluate(() => { document.querySelectorAll('[class*="toast"]').forEach(el => el.remove()) })
await page.screenshot({ path: 'screenshots/ar-templates.png', fullPage: false })
await browser.close()
console.log('saved screenshots/ar-templates.png')
